  ...
  if (newState === STATES.GENERACION) {
    ...
    if (session && session.photo) {
      ...
      comfyuiService.generate(
        session.photo,
        currentSessionId,
        // onStreamChunk — forward text outputs, save prompt_en for image workflow
        (chunk) => {
          console.log(`ComfyUI stream: ${chunk.channel} (${chunk.text_delta.length} chars)`);
          broadcast({ type: 'stream_chunk', ...chunk });
          if (chunk.channel === 'prompt_en' && chunk.done && currentSessionId) {
            sessionStore.set(currentSessionId, 'prompt_en', chunk.text_delta);
            console.log(`ComfyUI: saved prompt_en to session ${currentSessionId}`);
            console.log('─── PROMPT_EN ───');
            console.log(chunk.text_delta);
            console.log('─────────────────');
          }
        },
        // onPreview
        ...
        // onCannyReady — forward canny edge immediately when available
        (cannyData) => {
          console.log('ComfyUI: canny edge ready — broadcasting to frontend');
          broadcast({ type: 'canny_ready', image_b64: cannyData.image_b64 });
        },
        // onComplete
        (result) => {
